...
//        $this->useGoogleChrome(\SeleniumFinderRequest::CHROME_95);
    ...
    private function getData(string $url): ?array
    {
        ...
        $iframe = $this->waitForElement(\WebDriverBy::xpath("//iframe[contains(@src, '/_Incapsula_Resource?')]"), 0, true);
        ...
        $iframe = $this->waitForElement(\WebDriverBy::xpath("//iframe[contains(@src, '/_Incapsula_Resource?')]"), 5, true);
        ...
        if (!$dynamicContent) {
            try {
                $json = $this->driver->executeScript('
                    ...
                    xhr.open("GET", "https://digital.etihad.com/statics/applications/booking/dynamicContent/1.0.224/en.json", false);
